...
ko  <- ggplot(data=kosubset2, aes(x=date, y=kocount.below,  group=stream, colour=stream)) + geom_line() + geom_point()

# Code for other species plots. Counts are so low that these aren't really useful right now.
bsu <- ggplot(data=walks, aes(x=date, y=bsucount.below, group=stream, colour=stream)) + geom_line() + geom_point()
rb  <- ggplot(data=walks, aes(x=date, y=rbcount.below,  group=stream, colour=stream)) + geom_line() + geom_point()
bt  <- ggplot(data=walks, aes(x=date, y=btcount.below,  group=stream, colour=stream)) + geom_line() + geom_point()
...
plot(subset(kosubset2,stream=="truax creek")$date,subset(kosubset2,stream=="truax creek")$kocount.below,xlab="",ylab="Kokanee Count",
     las=1,xaxt="n",pch=19,type="b",col="black",ylim=c(0,120))
  points(subset(kosubset2,stream=="girl creek")$date,subset(kosubset2,stream=="girl creek")$kocount.below,xlab="",ylab="",
        las=1,xaxt="n",pch=19,type="b",col="red")
  points(subset(kosubset2,stream=="macdonald creek")$date,subset(kosubset2,stream=="macdonald creek")$kocount.below,xlab="",ylab="",
        las=1,xaxt="n",pch=19,type="b",col="darkgrey")
  points(subset(kosubset2,stream=="sucker creek")$date,subset(kosubset2,stream=="sucker creek")$kocount.below,xlab="",ylab="",
        las=1,xaxt="n",pch=19,type="b",col="blue")
  points(subset(kosubset2,stream=="marshall creek")$date,subset(kosubset2,stream=="marshall creek")$kocount.below,xlab="",ylab="",
...
plot(subset(rbsubset,stream=="marshall creek")$date,subset(rbsubset,stream=="marshall creek")$rbcount.below,xlab="",ylab="Rainbow Count",
     las=1,xaxt="n",pch=19,type="b",col="purple",ylim=c(0,12))
points(subset(rbsubset,stream=="girl creek")$date,subset(rbsubset,stream=="girl creek")$rbcount.below,xlab="",ylab="",
       las=1,xaxt="n",pch=19,type="b",col="red")
points(subset(rbsubset,stream=="macdonald creek")$date,subset(rbsubset,stream=="macdonald creek")$rbcount.below,xlab="",ylab="",
       las=1,xaxt="n",pch=19,type="b",col="darkgrey")
points(subset(rbsubset,stream=="truax creek")$date,subset(rbsubset,stream=="truax creek")$rbcount.below,xlab="",ylab="",
...
# The below code stopped working because I added the above and below columns. I'd have to go back and adjust.
# 
# for (i in 1:nrow(walks)) {
#   if (walks$kocount.below[i] > 0 | is.na(walks$kocount.below[i]))  walks$kopres[i]  <- 1 # At this time NA in the ko counts means they were seen at the hrb 
#   # bridge but no count was obtained due to turbidity.
#   if (walks$btcount.below[i] > 0)  walks$btpres[i]  <- 1
#   if (walks$bsucount.below[i] > 0) walks$bsupres[i] <- 1
#   if (walks$rbcount.below[i] > 0)  walks$rbpres[i]  <- 1
# }
# 
